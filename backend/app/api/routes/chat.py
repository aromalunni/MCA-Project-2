"""Chatbot API — smart customer support with predefined responses."""
...
RESPONSES = {
    ...
    r"(loyalty|point|reward|points|nff|ലോയൽറ്റി പോയിന്റുകൾ|പോയിന്റ്|പോയിന്റുകൾ)": (
        "Loyalty Program:\n"
    ...
    r"(hour|timing|open|close|when|samayam|time|സമയം)": (
        ...
        "സലൂൺ ഫൈൻഡറിൽ കൂടുതൽ വിവരങ്ങൾ കാണുക!"
    ),
    ...
    r"(help|support|issue|problem|complaint|സഹായം|പരാതി)": (
        ...
        "- പ്രശ്നങ്ങൾ ഉണ്ടെങ്കിൽ പേജ് റിഫ്രഷ് ചെയ്യുക\n"
...
def get_smart_response(message: str, db: Session, user_id: int) -> str:
    ...
    if re.search(r"(service|price|cost|how much|rate|seva|vila|സേവനങ്ങൾ|വില|വിലവിവരങ്ങൾ)", msg):
        ...
        if services:
            if is_ml:
                svc_list = "\n".join([f"- {s.name}: Rs.{s.price:.0f} ({s.duration_minutes} മിനിറ്റ്)" for s in services])
                return f"ഞങ്ങളുടെ പ്രധാന സേവനങ്ങൾ:\n{svc_list}\n\nകൂടുതൽ വിവരങ്ങൾക്ക് ബുക്ക് പേജ് കാണുക!"
            else:
                svc_list = "\n".join([f"- {s.name}: Rs.{s.price:.0f} ({s.duration_minutes} min)" for s in services])
                return f"Our popular services:\n{svc_list}\n\nVisit the Book page to see all!"
    ...
    # Dynamic: locations
    if re.search(r"(location|branch|address|where|find|salon|locate|evide|എവിടെ|ലൊക്കേഷൻ|ലൊക്കേഷനുകൾ|സലൂൺ ലൊക്കേഷനുകൾ)", msg):
        locations = db.query(Location).filter(Location.is_active == True).all()
        if locations:
            seen = set()
            loc_lines = []
            ml_translations = {
                "SmartSalon - MG Road Kochi": "സ്മാർട്ട് സലൂൺ - എം.ജി റോഡ് കൊച്ചി",
                "SmartSalon - Technopark": "സ്മാർട്ട് സലൂൺ - ടെക്നോപാർക്ക്, തിരുവനന്തപുരം",
                "SmartSalon - Calicut Beach": "സ്മാർട്ട് സലൂൺ - കോഴിക്കോട് ബീച്ച്",
                "SmartSalon - Thrissur Round": "സ്മാർട്ട് സലൂൺ - തൃശൂർ റൗണ്ട്",
                "SmartSalon - Kollam Beach": "സ്മാർട്ട് സലൂൺ - കൊല്ലം ബീച്ച്",
                "SmartSalon - Kottayam": "സ്മാർട്ട് സലൂൺ - കോട്ടയം",
                "SmartSalon - Alappuzha": "സ്മാർട്ട് സലൂൺ - ആലപ്പുഴ",
                "SmartSalon - Kannur City": "സ്മാർട്ട് സലൂൺ - കണ്ണൂർ സിറ്റി"
            }
            for l in locations:
                key = l.name.strip()
                if key not in seen:
                    seen.add(key)
                    if is_ml:
                        ml_name = ml_translations.get(key, key)
                        if "സ്മാർട്ട്" in ml_name and "-" in ml_name and "," not in ml_name and "തിരുവനന്തപുരം" not in ml_name and ml_name != key:
                             loc_lines.append(f"- {ml_name}")
                        elif ml_name != key:
                             loc_lines.append(f"- {ml_name}")
                        else:
                             loc_lines.append(f"- സ്മാർട്ട് സലൂൺ - {l.city}")
                    else:
    ...
    # Dynamic: staff
    if re.search(r"(staff|stylist|barber|who|aarun|സ്റ്റൈലിസ്റ്റ്|ആരുണ്ട്)", msg):
...
def send_message(
    ...
    response = get_smart_response(payload.message, db, user.id)

    # Save user message
    user_msg = ChatMessage(user_id=user.id, message=payload.message, is_bot=False)
    db.add(user_msg)

    # Save bot response
    bot_msg = ChatMessage(user_id=user.id, message=response, response=response, is_bot=True)
    db.add(bot_msg)
